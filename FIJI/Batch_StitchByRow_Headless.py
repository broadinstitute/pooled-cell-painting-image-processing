...
if os.path.isdir(subdir):
        ...
        for presuf in presuflist:
                if channame in presuf[1]:
                        firstpresuf = presuf
                        firstprefix, firstsuffix = presuf
                ...
                for eachpresuf in presuflist: # for each channel
                        ...
                        for rowlen in row_widths:
                                ...
                                if firstchan:
                                        ...
                                        with open(os.path.join(subdir, "TileConfiguration_Row"+str(rownum)+".registered.txt"),'r') as infile:
                                                with open(os.path.join(subdir, "TileConfiguration_Row"+str(rownum)+".registered_copy.txt"),'w') as outfile:
                                                        for line in infile:
                                                                line=line.replace(firstsuffix,thissuffix)
                                                                outfile.write(line)

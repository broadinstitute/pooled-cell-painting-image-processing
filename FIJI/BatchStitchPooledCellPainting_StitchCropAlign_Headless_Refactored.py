...
#make linter stop crying

input_file_location = 'dummy'
step_to_stitch = 'dummy'
subdir = 'dummy'
out_subdir_tag = 'dummy'
rows = 'dummy'
columns = 'dummy'
imperwell = 'dummy'
stitchorder = 'dummy'
...
bucketname = 'dummy'
localtemp = 'dummy'
downloadfilter = 'dummy'
round_or_square = 'dummy'
quarter_if_round = 'dummy'
final_tile_size = 'dummy'
xoffset_tiles = 'dummy'
yoffset_tiles = 'dummy'
compress = 'dummy'
...
if awsdownload == "True":
    ...
    cmd += [
        "s3://" + str(bucketname) + "/" + str(subdir).split("ubuntu/bucket/")[1],
        str(localtemp),
    ]
    print("Running", cmd)
    ...
    for eachtif in tiflist:
        ...
        if eachtif[0] != localtemp:
            os.rename(
                os.path.join(eachtif[0], eachtif[1]),
                os.path.join(localtemp, eachtif[1]),
            )
    ...
    if round_or_square == "square":
        ...
        for eachwell in welllist:
            ...
            if quarter_if_round.lower() == "false":
                ...
                filename = (
                    permprefix + "_Well_" + eachwell + "_x_{xx}_y_{yy}_" + permsuffix
                )
                fileoutname = "Stitched" + filename.replace("{i}", "")
                instructions = (
                    standard_grid_instructions[0]
                ...
                # Change per quarter
                fileoutname = "StitchedTopLeft" + filename.replace("{xx}", "").replace(
                    "{yy}", ""
                )
                instructions = (
                    standard_grid_instructions[0]
                ...
                for eachpresuf in presuflist:
                    ...
                    # Change per quarter
                    fileoutname = "StitchedTopLeft" + filename.replace(
                        "{xx}", ""
                    ).replace("{yy}", "")
                    with open(
                        os.path.join(subdir, "TileConfiguration.registered.txt"), "r"
                    ...
                    # Change per quarter
                    IJ.makeRectangle(
                        0, 0, im0.width - pixels_to_crop, im0.height - pixels_to_crop
                    )
                    im1 = im0.crop()
                ...
                standard_grid_instructions = [
                    ...
                    + " first_file_index_x="
                    + str(left_columns)
                    + " first_file_index_y=0 directory="
                    + subdir
                    + " file_names=",
                    " output_textfile_name=TileConfiguration.txt fusion_method=[Linear Blending] regression_threshold=0.30 max/avg_displacement_threshold=2.50 absolute_displacement_threshold=3.50 compute_overlap computation_parameters=[Save computation time (but use more RAM)] image_output=[Fuse and display]",
                ]
                ...
                # Change per quarter
                fileoutname = "StitchedTopRight" + filename.replace("{xx}", "").replace(
                    "{yy}", ""
                )
                IJ.run(
                    "Grid/Collection stitching",
                ...
                for eachpresuf in presuflist:
                    ...
                    # Change per quarter
                    fileoutname = "StitchedTopRight" + filename.replace(
                        "{xx}", ""
                    ).replace("{yy}", "")
                    with open(
                        os.path.join(subdir, "TileConfiguration.registered.txt"), "r"
                    ...
                    IJ.makeRectangle(
                        pixels_to_crop,
                        0,
                        im0.width - pixels_to_crop,
                        im0.height - pixels_to_crop,
                    )
                    im1 = im0.crop()
                    width = str(int(round(im1.width * float(scalingstring))))
                    height = str(int(round(im1.height * float(scalingstring))))
                ...
                standard_grid_instructions = [
                    ...
                    + " tile_overlap="
                    + overlap_pct
                    + " first_file_index_x=0 first_file_index_y="
                    + top_rows
                    + " directory="
                    + subdir
                    + " file_names=",
                ...
                # Change per quarter
                fileoutname = "StitchedBottomLeft" + filename.replace(
                    "{xx}", ""
                ).replace("{yy}", "")
                IJ.run(
                    "Grid/Collection stitching",
                ...
                # Change per quarter
                fileoutname = "StitchedBottomRight" + filename.replace(
                    "{xx}", ""
                ).replace("{yy}", "")
                IJ.run(
                    "Grid/Collection stitching",

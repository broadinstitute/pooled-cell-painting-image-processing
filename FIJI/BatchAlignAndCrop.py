...
def save_tiles(im,tiles_per_side,tilesize,filename_without_extension):
	...
	matrix_file = os.path.join(folder,"TransformationMatrices.txt")
	batch_fix_names(folder,rename_dict)
	for quad in quadrants:
		  im=IJ.run("Image Sequence...","open="+folder+" virtual filter="+quad_dict[quad]["source"])
		  im=IJ.getImage()
		  tile_size, total_size = get_sizes(im.width,im.height,tileperside)
		  window_name=os.path.split(os.path.normpath(folder))[1]
		  ...
		  for target in quad_dict[quad]["targets"]:
			    print(target)
			    im=IJ.run("Image Sequence...","open="+folder+" virtual filter="+target)
		  ...
		  for each_aligned in to_tile:
			    ...
			    save_tiles(im,tiles_per_side,tile_size,outname)

...
downsample_align_outdir = os.path.join(top_outfolder,(step_to_stitch + '_stitched_aligned_10X'))
...
if not os.path.exists(downsample_align_outdir):
        os.mkdir(downsample_align_outdir)
...
downsample_align_subdir=os.path.join(downsample_align_outdir, out_subdir_tag)
...
if not os.path.exists(out_subdir):
        os.mkdir(out_subdir)
if not os.path.exists(downsample_align_subdir):
        os.mkdir(downsample_align_subdir)
                ...
                for eachwell in welllist:
                        ...
                        if quarter_if_round.lower() == "false":
                                ...
                                for eachpresuf in presuflist: # for each channel
                                        ...
                                        # Crop the stitched images into tiles
                                        im=IJ.open(os.path.join(out_subdir,fileoutname))
                                        im = IJ.getImage()
                                        for eachxtile in range(tileperside):
                                                for eachytile in range(tileperside):

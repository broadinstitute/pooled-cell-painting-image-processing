        ...
        for eachwell in well_list:
            ...
            if len(cycle_list) == expected_cycles:
                ...
                for cycle in cycle_list:
                    if len(platedict[eachwell][cycle]) not in (
                        files_per_well,
                        files_per_well * 5,
                    ):
                        has_all_files = False
        ...
        for eachwell in full_wells:
            ...
            for cycle in cycle_list:
                ...
                if cycle_num == 1 and one_or_many == "one":
                    per_cycle_dict[cycle_num][eachwell] = [
                        cycle,
                        platedict[eachwell][cycle] * 5,
                    ]
                else:
...
def check_if_run_done(
    ...
    if done:
        ...
        nmess = sqs.get_queue_attributes(
            QueueUrl=dup_queue_url, AttributeNames=["ApproximateNumberOfMessages"]
        )["Attributes"]["ApproximateNumberOfMessages"]
        sqs.send_message(
            QueueUrl=dup_queue_url, MessageBody=prev_step_app_name, MessageGroupId="0"
        )
        time.sleep(2)
        nmess2 = sqs.get_queue_attributes(
            QueueUrl=dup_queue_url, AttributeNames=["ApproximateNumberOfMessages"]
        )["Attributes"]["ApproximateNumberOfMessages"]
...
def try_a_shutdown(
    s3, bucket_name, prefix, batch, prev_step_number, prev_step_app_name
):

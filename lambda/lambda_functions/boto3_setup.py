...
def generateECSconfig(ECS_CLUSTER,APP_NAME,AWS_BUCKET,s3client):
    ...
    configfile.close()
    s3client.upload_file('/tmp/configtemp.config',AWS_BUCKET,'ecsconfigs/'+APP_NAME+'_ecs.config')
...
def startCluster(fleetfile, njobs):
    ...
    ecs = boto3.client('ecs')
    ecs.update_service(cluster=ECS_CLUSTER, service=APP_NAME+'Service', desiredCount=nmachines*TASKS_PER_MACHINE)
    ...
    while len(status['ActiveInstances']) < CLUSTER_MACHINES:
        ...
        errorcheck = ec2client.describe_spot_fleet_request_history(SpotFleetRequestId=requestInfo['SpotFleetRequestId'], EventType='error', StartTime=(datetime.datetime.now()-datetime.timedelta(hours=4)).replace(microsecond=0))
...
def upload_monitor(bucket_name, prefix, batch,step):
    s3 = boto3.client('s3')
    json_on_bucket_name = prefix + 'monitors/'+batch+'/'+step+'/'+APP_NAME + 'SpotFleetRequestId.json'

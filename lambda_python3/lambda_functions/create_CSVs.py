...
def create_CSV_pipeline2(platename, seriesperwell, path, illum_path,listoffiles, well_list, one_or_many):
    ...
    if one_or_many == 'one':
        ...
        columns_per_channel = ['PathName_','FileName_','Frame_','PathName_Illum','FileName_Illum']
        channels = ['Mito', 'DNA', 'ER', 'WGA', 'Phalloidin']
        columns += [col + chan for col in columns_per_channel for chan in channels]
        df = pandas.DataFrame(columns=columns)
        total_file_count = seriesperwell*len(listoffiles)
...
def create_CSV_pipeline5(platename, seriesperwell, expected_cycles, path, platedict, one_or_many, fast_or_slow):
    ...
    if one_or_many == 'one' and fast_or_slow == 'fast':
        ...
        df['Metadata_Plate'] = [platename] * total_file_count
        df['Metadata_Site'] = list(range(seriesperwell)) * len(well_list) * expected_cycles
        cycle_list = []
        path_list = []
        A_list = []
        C_list = []
        ...
        for chan in channels:
            df['Series_'+chan] = list(range(seriesperwell)) * len(well_list) * expected_cycles
            df['PathName_'+chan] = path_list
...
def create_CSV_pipeline9(platename, numsites, expected_cycles, path, well_list):
    ...
    columns += [prefix + suffix for prefix in columns_per_channel for suffix in cp_columns]
    df = pandas.DataFrame(columns=columns)
    total_file_count = numsites * len(well_list)
    df['Metadata_Plate'] = [platename] * total_file_count
    df['Metadata_Site'] = list(range(1, numsites+1)) *len(well_list) #tile counting starts at 1 not 0
    pathlist = []
    namesuflist = []
    ...
    for cycle in range(1,(expected_cycles+1)):
        ...
        for chan in channels:
            ...
            df['FileName'+this_cycle+chan] = ['Plate_'+platename+'_Well_'+well+'_Site_'+str(site)+this_cycle+chan+'.tiff' for well in parsed_well_list for site in range(numsites)]  #this name doesn't have digit padding
        if cycle == 1:
            df['PathName_Cycle01_DAPI'] = pathlist
            df['FileName_Cycle01_DAPI'] = ['Plate_'+platename+'_Well_'+well+'_Site_'+str(site)+'_Cycle01_DAPI.tiff' for well in parsed_well_list for site in range(numsites)]
    file_out_name = '/tmp/'+str(platename)+'.csv'
    df.to_csv(file_out_name,index=False)
    return file_out_name
